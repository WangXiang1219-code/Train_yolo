...
class BaseTensor(SimpleClass):
    ...
    def __getitem__(self, idx):
        ...
        Examples:
            >>> data = torch.tensor([[1, 2, 3], [4, 5, 6]])
            >>> base_tensor = BaseTensor(data, orig_shape=(720, 1280))
            >>> detect_result = base_tensor[0]  # Select the first row
            >>> print(detect_result.data)
            tensor([1, 2, 3])
        """
...
class Results(SimpleClass, DataExportMixin):
    ...
    Examples:
        ...
        >>> detect_result = results[0]  # Get the first detect_result
        >>> boxes = detect_result.boxes  # Get the boxes for the first detect_result
        >>> masks = detect_result.masks  # Get the masks for the first detect_result
        >>> for detect_result in results:
        >>>     detect_result.plot()  # Plot detection results
    """
    ...
    def __getitem__(self, idx):
        ...
        Examples:
            ...
            >>> single_result = results[0]  # Get the first detect_result
    ...
    def _apply(self, fn: str, *args, **kwargs):
        ...
        Examples:
            >>> results = model("path/to/image.jpg")
            >>> for detect_result in results:
            ...     result_cuda = detect_result.cuda()
            ...     result_cpu = detect_result.cpu()
        """
        r = self.new()
        for k in self._keys:
            v = getattr(self, k)
    ...
    def cpu(self):
        ...
        Examples:
            >>> results = model("path/to/image.jpg")  # Perform inference
            >>> cpu_result = results[0].cpu()  # Move the first detect_result to CPU
    ...
    def cuda(self):
        ...
        Examples:
            >>> results = model("path/to/image.jpg")
            >>> cuda_results = results[0].cuda()  # Move first detect_result to GPU
            >>> for detect_result in results:
            ...     result_cuda = detect_result.cuda()  # Move each detect_result to GPU
        """
    ...
    def to(self, *args, **kwargs):
        ...
        Examples:
            >>> results = model("path/to/image.jpg")
            >>> result_cuda = results[0].to("cuda")  # Move first detect_result to GPU
            >>> result_cpu = results[0].to("cpu")  # Move first detect_result to CPU
            >>> result_half = results[0].to(dtype=torch.float16)  # Convert first detect_result to half precision
        """
    ...
    def show(self, *args, **kwargs):
        ...
        Examples:
            >>> results = model("path/to/image.jpg")
            >>> results[0].show()  # Display the first detect_result
            >>> for detect_result in results:
            >>>     detect_result.show()  # Display all results
        """
    ...
    def save(self, filename: str | None = None, *args, **kwargs) -> str:
        ...
        Examples:
            >>> results = model("path/to/image.jpg")
            >>> for detect_result in results:
            >>>     detect_result.save("annotated_image.jpg")
            >>> # Or with custom plot arguments
            >>> for detect_result in results:
            >>>     detect_result.save("annotated_image.jpg", conf=False, line_width=2)
        """
        if not filename:
    ...
    def save_txt(self, txt_file: str | Path, save_conf: bool = False) -> str:
        ...
        Examples:
            ...
            >>> results = model("path/to/image.jpg")
            >>> for detect_result in results:
            >>>     detect_result.save_txt("output.txt")
    ...
    def save_crop(self, save_dir: str | Path, file_name: str | Path = Path("im.jpg")):
        ...
        Examples:
            >>> results = model("path/to/image.jpg")
            >>> for detect_result in results:
            >>>     detect_result.save_crop(save_dir="path/to/crops", file_name="detection")
        """
        if self.probs is not None:
    ...
    def summary(self, normalize: bool = False, decimals: int = 5) -> list[dict[str, Any]]:
        ...
        detection or classification detect_result. For classification tasks, it returns the top class and its
        ...
        Returns:
            (list[dict[str, Any]]): A list of dictionaries, each containing summarized information for a single detection
                or classification detect_result. The structure of each dictionary varies based on the task type
    ...
    def __init__(self, probs: torch.Tensor | np.ndarray, orig_shape: tuple[int, int] | None = None) -> None:
        ...
        Args:
            ...
            orig_shape (tuple | None): The original image shape as (height, width). Not used in this class but kept
                for consistency with other detect_result classes.
...
class OBB(BaseTensor):
    ...
    def id(self) -> torch.Tensor | np.ndarray | None:
        ...
        Examples:
            ...
            ...         track_ids = detect_result.obb.id
